...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const loadProduct = async (silent = false) => {
        ...
        const { data, error } = await supabase
            ...
            .select(`
                *,
                store:stores(id, name, slug, currency_symbol, currency_position, custom_domain),
        ...
        if ((productData as any).variant_selection_title) {
            setVariantSelectionTitle((productData as any).variant_selection_title);
        }
        if ((productData as any).variants && Array.isArray((productData as any).variants)) {
            setVariants((productData as any).variants);
            setHasVariants((productData as any).variants.length > 0);
        }
        if ((productData as any).variant_prices) {
            setVariantPrices((productData as any).variant_prices);
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (newImages.length > 0) {
                ...
                const { error: insertError } = await supabase.from('product_images').insert(imageInserts);
    ...
    const previewUrl = store?.custom_domain
        ? `https://${store.custom_domain}.${rootDomain}/products/${product.slug}`
        : `https://${rootDomain}/${store?.slug}/${product.slug}`;
                                                            ...
                                                            {getVariantCombinations().map((combo) => (
                                                                <tr key={combo} className="bg-white">
                                                                    ...
                                                                    <td className="px-4 py-3">
                                                                        <div className="relative">
                                                                            <span className="absolute left-2 top-1/2 -translate-y-1/2 text-gray-400 text-xs">{store?.currency_symbol || '$'}</span>
                                                                            <Input
                                                                                type="number"
