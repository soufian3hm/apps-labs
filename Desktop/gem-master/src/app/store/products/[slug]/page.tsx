...
import { submitLead } from '@/app/actions/orders';
...
import type { CustomElement } from '@/components/form-builder/custom-elements';
...
function CustomElementRenderer({ element, formData, setFormData }: {
    ...
    switch (element.type) {
        ...
        case 'social_links':
            return (
                <div className="flex justify-center gap-3 py-2">
                    {element.socialLinks?.map((link: { platform: string; url: string; icon?: string }, i: number) => (
                        <a
                            key={i}
                            href={link.url}
                            target="_blank"
        ...
        case 'accordion':
            return (
                <div className="space-y-2">
                    {element.accordionItems?.map((item: { id: string; title: string; content: string }, i: number) => (
            ...
            return (
                <div className="space-y-4">
                    <div className="flex gap-2 border-b">
                        {element.tabItems?.map((tab: { id: string; title: string; content: string }, i: number) => (
                            <button
                                key={i}
                                type="button"
                        ...
                        {variant.values.map((val) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`px-4 py-2 text-sm rounded-lg border-2 transition-all duration-200 ${isSelected
                                        ? 'ring-2 ring-offset-1 font-medium shadow-sm'
                                        : 'hover:bg-gray-50'
                                        }`}
                                    style={{
                                        borderColor: isSelected ? '#22c55e' : '#e5e7eb',
                                        backgroundColor: isSelected ? '#22c55e' : '#ffffff',
                                        color: isSelected ? '#ffffff' : styles.textColor,
                                    }}
                                >
                                    {val}
                                </button>
...
function EnhancedLeadForm({
    ...
    onVariantSelect: (name: string, value: string) => void;
    price: number;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if ((product as any).variants) {
            const missing = (product as any).variants.find((v: any) => !selectedVariants[v.name]);
            if (missing) {
        ...
        try {
            ...
            const result = await submitLead(product.id, submissionData);
    ...
    const replaceVariablesLocal = (text: string) => {
        if (!text) return '';
        const formattedPrice = formatPrice(price, store.currency_symbol, store.currency_position);
        return text
            .replace(/{{product_name}}/g, product.name)
            .replace(/{product_name}/g, product.name)
            .replace(/{{price}}/g, formattedPrice)
            .replace(/{price}/g, formattedPrice)
            .replace(/{{total}}/g, formattedPrice)
            .replace(/{total}/g, formattedPrice)
            .replace(/{{compare_price}}/g, comparePrice ? formatPrice(comparePrice, store.currency_symbol, store.currency_position) : '')
            .replace(/{compare_price}/g, comparePrice ? formatPrice(comparePrice, store.currency_symbol, store.currency_position) : '');
    };
                            ...
                            {block.type === 'summary' && (
                                ...
                                >
                                    ...
                                    <span className="font-bold">
                                        {formatPrice(price, store.currency_symbol, store.currency_position)}
                                    </span>
                                </div>
                            )}
                                ...
                                >
                                    ...
                                    <div className="space-y-2 text-sm">
                                        <div className="flex justify-between">
                                            ...
                                            <span>{formatPrice(price, store.currency_symbol, store.currency_position)}</span>
                                        </div>
                            ...
                            {block.type === 'button' && (
                                ...
                                >
                                    {isSubmitting ? (
                                        ...
                                        replaceVariablesLocal(block.content || 'Order Now')
                                    )}
                                </Button>
                            )}
                        </div>
...
export default function StoreProductPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const currentPrice = useMemo(() => {
        if (!product) return { price: 0, compare_price: undefined as number | undefined };
        let p = product.price;
        let c: number | undefined = product.compare_price;

        if ((product as any).variant_prices && Object.keys(selectedVariants).length > 0 && (product as any).variants) {
            const parts = (product as any).variants.map((v: any) => selectedVariants[v.name]).filter(Boolean);
            // Only update calculation if all variants are selected? 
            // Or try match partial?
            // Usually we only update if FULL KEY matches. 
            // Admin generates full keys "A / B". 
            // If user selected "A" but not "B", key "A" won't match "A / B".
            // So logic works: only perfect match updates price.

            const key = parts.join(' / ');
            const variantPrice = (product as any).variant_prices[key];
            if (variantPrice) {
                if (variantPrice.price) p = parseFloat(variantPrice.price);
                // If variant has price, it overrides.

                if (variantPrice.compare_price) {
                    c = parseFloat(variantPrice.compare_price);
                } else if (variantPrice.price) {
                    // If variant has price but NO compare_price, should we reset compare_price?
                    // Typically yes, or inherit? 
                    // Let's assume reset unless specified.
                    c = undefined;
                }
            }
        }
        return { price: p, compare_price: c };
    }, [product, selectedVariants]);

    // Initial subdomain check
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const currentDomain = storeDomain;
        ...
        async function fetchData() {
            ...
            if (productError || !productBase) {
                setNotFound(true);
            } else {
                // Fetch related data in parallel since they don't have direct FKs to public_products
                const [images, specs, leadForm] = await Promise.all([
                    supabase.from('product_images').select('*').eq('product_id', productBase.id!).order('sort_order'),
                    supabase.from('product_specifications').select('*').eq('product_id', productBase.id!).order('sort_order'),
                    supabase.from('lead_form_configs').select('*, fields:lead_form_fields(*)').eq('product_id', productBase.id!).maybeSingle()
                ]);
                ...
                const fullProduct = {
                    ...
                    lead_form_config: leadForm.data || null
                };

                setProduct(fullProduct as unknown as ProductWithRelations);
            }
    ...
    if (notFound || !product || !store) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4 font-sans">
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 text-center max-w-md w-full animate-in fade-in zoom-in-95 duration-300">
                    <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4">
                        <ShoppingCart className="w-8 h-8 text-gray-400" />
                    </div>
                    <h1 className="text-xl font-bold text-gray-900 mb-2">Product Not Available</h1>
                    <p className="text-gray-500 mb-6 text-sm">
                        The product you are looking for does not exist or has been removed.
                    </p>
                    <Link
                        href="/"
                        className="inline-flex items-center justify-center h-10 px-6 text-sm font-medium text-white transition-colors bg-gray-900 rounded-lg hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2"
                    >
                        Return Home
                    </Link>
                </div>
    ...
    const renderSection = (sectionId: string, isDesktopRightColumn = false) => {
        ...
        switch (sectionId) {
            ...
            case 'price':
                return (
                    <div key="price" className="flex items-center justify-center gap-3 mb-6 animate-in fade-in duration-300">
                        <span className="text-3xl font-bold transition-all" style={{ color: theme.primary_color }}>
                            {formatPrice(currentPrice.price, currencySymbol, currencyPosition)}
                        </span>
                        {currentPrice.compare_price && (
                            <span className="text-xl line-through opacity-50 transition-all" style={{ color: theme.text_color }}>
                                {formatPrice(currentPrice.compare_price, currencySymbol, currencyPosition)}
                            </span>
                        )}
                    </div>
                );
                ...
                return (
                    <div key="lead_form" className="mb-8">
                        <EnhancedLeadForm
                            ...
                            onVariantSelect={(name, value) => setSelectedVariants(prev => ({ ...prev, [name]: value }))}
                            price={currentPrice.price}
                            comparePrice={currentPrice.compare_price}
                        />
                    </div>
                );
            case 'description':
    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto md:p-6 lg:p-8 flex-1 w-full flex flex-col">
                {/* Desktop Grid Layout */}
                <div className="hidden lg:grid lg:grid-cols-12 lg:gap-12 lg:items-start">
                    ...
                    <div className="lg:col-span-5 space-y-2 flex flex-col h-full">
                ...
                <div className="lg:hidden flex flex-col flex-1 pb-20" style={{ padding: '20px' }}>
